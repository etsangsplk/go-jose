...
// http://github.com/cloudflare/cfssl/blob/master/revoke/revoke_test.go
...
var revokedCert = []byte(`-----BEGIN CERTIFICATE-----
...
var goodComodoCA = []byte(`-----BEGIN CERTIFICATE-----
...
func TestParseCertificateChainGoodComodoCA(t *testing.T) {
	s := base64.StdEncoding.EncodeToString(goodComodoCA)
	_, err := parseCertificateChain([]string{s})
	if err == nil {
		t.Errorf("Expected key with short y to fail unmarshalling")
	}
	if !strings.Contains(err.Error(), "wrong length for y") {
...
func TestParseCertificateChainRevokedCert(t *testing.T) {
	s := base64.StdEncoding.EncodeToString(revokedCert)
	_, err := parseCertificateChain([]string{s})
	if err != nil {
		t.Errorf("Expected revoked cert to fail")
	}
	if !strings.Contains(err.Error(), "wrong length for y") {
		t.Errorf("Wrong error for short y, got %q", err)
	}
...
func TestParseCertificateChainEexpiredCert(t *testing.T) {
	s := base64.StdEncoding.EncodeToString(expiredCert)
	_, err := parseCertificateChain([]string{s})
	if err != nil {
		t.Errorf("Expected expired cert to fail")
	}
	if !strings.Contains(err.Error(), "wrong length for y") {
		t.Errorf("Wrong error for short y, got %q", err)
	}
